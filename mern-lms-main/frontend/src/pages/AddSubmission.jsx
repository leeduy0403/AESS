...
export default function AddSubmission() {
  ...
  const [maxNumberOfFile, setMaxNumberOfFile] = useState(0);
  ...
  useEffect(() => {
    ...
    const fetchMaxFileSize = async () => {
      try {
        const res = await fetch(
          `/api/assignment/get-assignment-max-file-size/${assignmentId}`
        );
        const data = await res.json();
        if (res.ok) {
          console.log("Max file size limit: ", data); //? debug
          setMaxFileSize(data.maxSize);
          setMaxNumberOfFile(data.maxNumberOfFile);
        }
      } catch (error) {
        console.log(error.message); //TODO: need fix for "Unexpected token '<', "<!doctype "... is not valid JSON"
      }
    };
    fetchMaterials();
    fetchMaxFileSize();
  ...
  const handleFilesSubmit = (e) => {
    ...
    console.log("Max file size: ", maxFileSize); //? debug
    if (files.length > maxNumberOfFile) {
      setFilesUploadError("File count exceeds the maximum limit");
      setUploading(false);
      setValidForm(false);
      return;
    }
    for (let i = 0; i < files.length; i++) {
      if (files[i].size > maxFileSize * 1024 * 1024) {
        setFilesUploadError("File size exceeds the maximum limit");
        setUploading(false);
        setValidForm(false);
        return;
      }
    }
    if (files.length > 0 && files.length + formData.submissionUrls.length < 7) {
      setUploading(true);
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          ...
          console.log("AddSubmission.js: ", err);
          setUploading(false);
        });
    } else {
      setUploading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`/api/submission/create/${assignmentId}`, {
        method: "POST",
        headers: {
      ...
      if (data.success === false) {
        ...
        setCreateSuccess("Add submission successfully!");
        ...
        <div className="basis-9/12">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-4">
            ...
            <div className="flex flex-col flex-1 gap-4">
              <div className="flex flex-col gap-1">
                <span>{`Maximum ${maxNumberOfFile} file(s). ${maxFileSize} MB per file.`}</span>
                <input
