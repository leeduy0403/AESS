...
export default function AddAssignment() {
  ...
  const [rubricFilesUploadError, setRubricFilesUploadError] = useState(false);
  ...
  const handleDescriptionFilesSubmit = (e) => {
    if (
      ...
      setDescriptionFilesUploadError(false);
      const promises = [];
      for (let i = 0; i < descriptionFiles.length; i++) {
  ...
  const handleRubricFilesSubmit = (e) => {
    if (
      ...
      setRubricFilesUploadError(false);
      const promises = [];
      for (let i = 0; i < rubricFiles.length; i++) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/assignment/create/${classId}`, {
        method: "POST",
        headers: {
      ...
      if (data.success === false) {
        ...
        setCreateSuccess("Add assignment successfully!");
                      ...
                      <RadioGroup
                        ...
                        onChange={(e) =>
                          setFormData((prev) => ({
                            ...prev,
                            type: e.target.value,
                            maxMemberGroup: e.target.value === "Group" ? 2 : 1,
                            startDateGroup: "",
                            endDateGroup: "",
                          }))
                        }
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex flex-col gap-1 flex-1">
                    <div className="flex items-center">
                      ...
                      />
                      <a
                        className="text-lg ml-auto text-blue-500 hover:underline"
                        target="_blank"
