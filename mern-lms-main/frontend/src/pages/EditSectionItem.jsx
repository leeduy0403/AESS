...
export default function EditSectionItem() {
  ...
  useEffect(() => {
    ...
    const tabFromUrl = urlParams.get("sectionId");
    if (tabFromUrl) {
      setSectionId(tabFromUrl);
    }
    const fetchMaterials = async () => {
      ...
      try {
        const resSection = await fetch(
          `/api/section/get/${classId}?sectionId=${sectionId}`
        );
        const section = await resSection.json();
        if (resSection.ok) {
          setSection(section[0]);
        }
        const resMaterial = await fetch(
          `/api/material/get-materials-section/${sectionId}?materialId=${materialId}`
        );
        const material = await resMaterial.json();
        if (resMaterial.ok) {
          setMaterialInfo(material);
          setNameFiles(material[0]?.nameFiles);
        }
      } catch (error) {
    ...
    fetchMaterials();
  }, [location.search, sectionId, classId, materialId]);

  useEffect(() => {
    if (materialInfo?.length > 0) {
      setFormData({
        description: materialInfo[0]?.description || "",
        materialUrls: materialInfo[0]?.materialUrls || "",
      });
    }
  }, [materialInfo]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/material/update/${materialId}`, {
        method: "PUT",
        headers: {
