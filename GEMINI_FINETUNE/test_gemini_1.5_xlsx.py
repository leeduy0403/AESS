...
def extract_scores_and_feedback(response_text, min_score, max_score):
    """Extracts component scores & calculates average overall score."""
    ...
    # ✅ Compute average score (rounded to nearest integer)
    ovr = round((coherence + lexical + grammar) / 3)
...
def evaluate_essays(description_path, content_path, output_path):
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            #     f"Step 5: Assign a score within the range of {min_score}-{max_score} for each category.\n"
            #     f"Step 6: Provide final feedback that is clear, actionable, and free from embedded numbers or scores.\n\n"

            #     f"Example:\n"
            #     f"Step 1: The essay presents strong arguments but lacks smooth transitions.\n"
            #     f"Step 2: Coherence and Cohesion – The ideas are logically structured, but certain sentences feel disconnected.\n"
            #     f"Step 3: Lexical Resource – The vocabulary is diverse, but some word choices are repetitive.\n"
            #     f"Step 4: Grammatical Range and Accuracy – Mostly correct grammar with a few minor errors.\n"
            #     f"Step 5: Scores: 9 \n"
            #     f"Coherence and Cohesion: 8 \n"
            #     f"Lexical Resource: 9 \n"
            #     f"Grammatical Range and Accuracy: 10 \n"
            #     f"Step 6: Feedback: The essay presents clear arguments and a strong structure. Focus on improving transitions and varying word choices to enhance clarity.\n\n"
            ...
            score, scores, feedback = extract_scores_and_feedback(response_text, min_score, max_score)

            results.append({
                "essay_id": essay_id,
                "ovr": score,  # ✅ Average of 3 component scores
                "scores": scores,
                "components": [
...
if __name__ == "__main__":
    evaluate_essays(
        ...
        "C://Users//Admin//OneDrive//DACN+DATN//AI MODEL//AES//OUTPUT//one_shot//output_gemini1.5_set1.xlsx"
    )
    evaluate_essays(
